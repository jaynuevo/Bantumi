...
public class AIEvaluate {
	...
	public int checkTurn(int d, int p1BowlCopy[], int p2BowlCopy[], int alpha, int beta){
		...
		if(d==3){
			...
			/*for (int i = 0; i<6; i++){
				if (Board.p2Bowl[maxIndex] == 0){
					scoreList.remove(scoreList.indexOf(Collections.max(scoreList)));
					maxIndex = scoreList.indexOf(Collections.max(scoreList));
					System.out.println("SECOND MAX IS" +scoreList.get(maxIndex));
				}
